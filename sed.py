...
with st.sidebar:
    ...
    st.markdown("`v9.1 | CS2 Fix Edition`")
    st.divider()
    menu_choice = st.radio("РОЗДІЛИ:", ["🔍 Dota 2 Сканер", "🔫 CS2 Скан", "💼 Портфель", "📊 Звіти (База)"])
    st.divider()
    ...
    if st.session_state.dota_result:
        ...
        with col_img:
            if res['set_info']["image_url"]:
                try:
                    img_res = session.get(res['set_info']["image_url"], timeout=5)
                    if img_res.status_code == 200: st.image(img_res.content, use_container_width=True)
                except: st.info("📷 Помилка завантаження фото")
        ...
        with col_info:
            ...
            st.markdown(f"<a href='{get_steam_client_url(res['exact_name'], STEAM_APP_ID_DOTA)}' style='display: inline-block; padding: 8px 15px; background-color: #1a2838; color: #66c0f4; text-decoration: none; border-radius: 5px; border: 1px solid #101822;'>Відкрити бандл у Steam 🔗</a>", unsafe_allow_html=True)
        ...
        with tab1:
            ...
            user_buy_price = col_in1.number_input("За скільки КУПИВ всі частини (сума):", value=float(res['total_parts_price']), step=1.0, key="buy_pack")
            user_sell_price = col_in2.number_input("За скільки ПРОДАШ цілий бандл (брутто):", value=float(res['bundle_data']['price']), step=1.0, key="sell_pack")
            ...
            with btn_col1:
                if st.button("📊 Додати у ЗВІТ (Вже продано)", type="primary", use_container_width=True, key="rep_pack"):
                    if user_buy_price > 0:
            ...
            with btn_col2:
                if st.button("💼 Додати у ПОРТФЕЛЬ (Ще лежить)", use_container_width=True, key="port_pack"):
                    if user_buy_price > 0:
        ...
        with tab2:
            col_in1, col_in2 = st.columns(2)
            user_bundle_buy = col_in1.number_input("За скільки КУПИВ цілий бандл:", value=float(res['bundle_data']['price']), step=1.0, key="buy_unpack")
            user_parts_sell = col_in2.number_input("За скільки ПРОДАШ всі деталі (брутто):", value=float(res['total_parts_price']), step=1.0, key="sell_unpack")
            ...
            btn_col1, btn_col2 = st.columns(2)
            with btn_col1:
                if st.button("📊 Додати у ЗВІТ (Вже продано)", type="primary", use_container_width=True, key="rep_unpack"):
                    if user_bundle_buy > 0:
                        save_to_reports(res['exact_name'], "Розпакування", int(user_bundle_buy), int(actual_unpack_profit))
                        st.success("✅ Записано в базу Звітів!")
                    else: st.warning("Введи ціну закупки!")
            with btn_col2:
                if st.button("💼 Додати у ПОРТФЕЛЬ (Ще лежить)", use_container_width=True, key="port_unpack"):
                    if user_bundle_buy > 0:
...
# ==========================================
    ...
    # Placeholder підказує, що регістр і дужки обов'язкові
    ...
    if search_cs_btn and cs_item_raw:
        cs_item = cs_item_raw.strip() # Прибираємо випадкові пробіли по краях
    ...
    if st.session_state.cs_result:
        ...
        if res['data']['price'] > 0 or res['data']['median'] > 0:
            ...
            col_in1, col_in2 = st.columns(2)
            p_buy = col_in1.number_input("Твоя ціна закупки (₴):", value=float(res['data']['price']), step=1.0)
            p_target = col_in2.number_input("Твоя цільова ціна продажу (₴):", value=float(res['data']['price']*1.2), step=1.0)
            
            btn_col1, btn_col2 = st.columns(2)
            with btn_col1:
                if st.button("📊 Додати у ЗВІТ (Вже продано)", type="primary", use_container_width=True, key="rep_cs"):
                    cs_profit = get_clean_income(p_target) - p_buy
                    save_to_reports(res['name'], "CS2 Трейд", int(p_buy), int(cs_profit))
                    st.success("✅ Записано в базу Звітів!")
            with btn_col2:
                if st.button("💼 Додати у ПОРТФЕЛЬ (Ще лежить)", use_container_width=True, key="port_cs"):
            ...
            st.info("💡 Підказка: скопіюй назву прямо з ринку Steam. Регістр (AK-47, а не ak-47) і якість (Field-Tested) — обов'язкові.")

# ==========================================
# СТОРІНКА 3: ПОРТФЕЛЬ (НОВА ВЕРСІЯ З УПРАВЛІННЯМ)
# ==========================================
